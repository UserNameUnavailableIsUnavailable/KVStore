    ...
    do
    {
        ...
        if (cmd.has_value())
        {
            ...
            else
            {
                ...
                std::ptrdiff_t sent = send(socket_.get_native_handle(), serialized.data(), serialized.size(), 0);
                if (sent < 0)
                {
                ...
                std::ptrdiff_t received = recv(socket_.get_native_handle(), buffer, sizeof(buffer) - 1, 0);
                if (received < 0)
                {
