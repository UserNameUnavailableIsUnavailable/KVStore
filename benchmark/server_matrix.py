...
def parse_args():
    ...
    # Layer 2 only: which malloc backs the process is selected outside the
    # program, by linking or LD_PRELOAD, so it is not part of this matrix.
    parser.add_argument("--allocators", nargs="+", choices=("default", "pool"),
                        default=("default",))
    ...
    for allocator in metadata["allocators"]:
        for value_size in metadata["value_sizes_bytes"]:
            ...
            lines.append(f"| **{value_size} B / {allocator}** | **mean across stores** | "
                         f"**{average_delta:+.2f}%** |")
    if "default" in metadata["allocators"] and "pool" in metadata["allocators"]:
        lines.extend([
            "", "## Allocation Strategy Difference", "",
            "Positive values mean the PMR pool achieved higher QPS than allocating "
            "straight from the process allocator.", "",
            "| Value | Store | Backend | pool QPS delta |", "|---:|---|---|---:|",
        ])
        for value_size in metadata["value_sizes_bytes"]:
            deltas = []
            for store in STORES:
                for backend in BACKendS:
                    delta = pool_delta(summaries, backend, store, value_size)
                    deltas.append(delta)
