...
- `--allocator default|pool`：选择分配策略
- `--memory-pooling` 和 `--memory-pool-size <bytes>`：启用预分配内存池

内存管理分三层，互相正交：

1. **进程分配器**：`malloc`/`free`（即 `new`/`delete` 的底层）。替换成 jemalloc 或 tcmalloc
   属于链接期或部署期的选择，不需要改代码：`LD_PRELOAD=libjemalloc.so.2 ./build/Server/KVServer`。
