...
public:
    ...
    bool SetTimeToLive(const std::string& key, std::chrono::duration<Rep, Period> ttl)
    {
        ...
        record->SetTimeToLive(std::optional {ttl});
        return true;
    }
};
...
TEST(DataStoreTesting, ExpiredRecordsAreDroppedOnAccess)
{
    TtlLruStore store;
    store.Set("key", "value");
    ASSERT_TRUE(store.SetTimeToLive("key", std::chrono::milliseconds(10)));
    EXPECT_TRUE(store.Exists("key"));
    EXPECT_EQ(store.Size(), 1u);
...
TEST(DataStoreTesting, RecycledNodeGetsAFreshLifecycle)
{
    TtlLruStore store(1);
    store.Set("a", "1");
    ASSERT_TRUE(store.SetTimeToLive("a", std::chrono::milliseconds(10)));
    std::this_thread::sleep_for(std::chrono::milliseconds(30));
