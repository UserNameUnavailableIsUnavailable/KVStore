...
public:
    ...
    {
        return store_.SetTimeToLive(key, ttl);
    }

    std::optional<std::chrono::milliseconds> TimeToLive(const std::string& key) override
    {
        return store_.GetTimeToLive(key);
    }

    std::vector<SnapshotEntry> Snapshot() override
    {
        std::vector<SnapshotEntry> entries;
        store_.VisitLive([&entries](const std::string& key, const std::string& value,
...
{
	return server_socket_.GetNativeHandle();
}
...
void Server::RecordReplicationCommand(const Command& command)
{
    ...
    {
        const int handle = (*it)->GetSocket().GetNativeHandle();
        std::size_t sent = 0;
...
{
    const int connection = session.GetSocket().GetNativeHandle();
    if (connection < 0)
    {
        return;
    }
...
bool Server::SynchronizeFromMaster(const std::string& address, std::uint16_t port)
{
    ...
    {
        const std::ptrdiff_t written = ::send(master_session_->GetSocket().GetNativeHandle(), encoded.data() + sent,
    ...
    while (true)
    {
        const std::ptrdiff_t received = ::recv(master_session_->GetSocket().GetNativeHandle(), buffer.data(), buffer.size(), 0);
        if (received <= 0)
        {
            master_session_.reset();
            return false;
