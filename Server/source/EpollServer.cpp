...
EpollMessageQueue::EpollMessageQueue()
{
    ...
    ::epoll_event event {.events = EPOLLIN, .data = {.fd = timers_.GetNativeHandle()}};
    if (::epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, timers_.GetNativeHandle(), &event) < 0)
    {
        ::close(epoll_fd_);
...
{
    pending_.insert_or_assign(task.GetHandle(), &task);
    if (!Arm(task.GetHandle(), EPOLLIN))
    {
        const int error = -errno;
        pending_.erase(task.GetHandle());
        task.Complete(error);
        ready_.push_back({.type = MessageType::kRead,
            .session_id = static_cast<std::size_t>(task.GetHandle()),
            .result = error,
...
{
    pending_.insert_or_assign(task.GetHandle(), &task);
    if (!Arm(task.GetHandle(), EPOLLOUT))
    {
        const int error = -errno;
        pending_.erase(task.GetHandle());
        task.Complete(error);
...
{
    const Socket::HandleType handle = task.GetHandle();
...
{
    pending_.erase(task.GetHandle());
    ::epoll_ctl(epoll_fd_, EPOLL_CTL_DEL, task.GetHandle(), nullptr);
}
...
std::optional<Message> EpollMessageQueue::ProcessEvent(const ::epoll_event& event)
{
    ...
    if (handle == timers_.GetNativeHandle())
    {
    ...
    int result = std::visit([&message](auto* task) -> int {
        ...
        if constexpr (std::is_same_v<TaskType, ReceiveOperation>)
        {
            ...
            return static_cast<int>(::recv(task->GetHandle(), buffer.data(), buffer.size(), 0));
        }
        else if constexpr (std::is_same_v<TaskType, SendOperation>)
        {
            ...
            return static_cast<int>(::send(task->GetHandle(), buffer.data(), buffer.size(), MSG_NOSIGNAL));
        }
        else
        {
            ...
            if (::getsockopt(task->GetHandle(), SOL_SOCKET, SO_ERROR, &error, &size) < 0)
            {
                return -errno;
            }
...
void EpollServer::Run(std::uint16_t port, int backlog)
{
    ...
    const Socket::HandleType listener_handle = server_socket_.GetNativeHandle();
...
void EpollServer::DispatchMessage(const Message& message)
{
        ...
        message.session_id == static_cast<std::size_t>(master_session_->GetSocket().GetNativeHandle()))
    {
        ...
        if (replication_task_->Done())
        {
            ...
            message_queue_.Unregister(master_session_->GetSocket().GetNativeHandle());
            replication_task_.reset();
            master_session_.reset();
        }
        return;
