...
// peer is simply gone.
std::string LinkState(NBIO::RDMA_Session &session)
{
...
// concern, and reporting it is what poll_send is for.
std::error_code SendMessage(NBIO::RDMA_Session &session, std::span<const char> message)
{
    auto chunk = session.send_channel().acquire();
    if (!chunk) [[unlikely]]
    {
    ...
    return session.send(*chunk, message.size());
}
...
NBIO::Task<bool> SendControl(NBIO::RDMA_Session &session, std::span<const char> message)
{
    while (true)
    {
        const auto failure = SendMessage(session, message);
        if (!failure)
        {
            co_return true;
        }
        if (failure != std::errc::no_buffer_space) [[unlikely]]
        {
            spdlog::warn("transfer: sending a control message failed: {}", failure.message());
            co_return false;
        }
        ...
        if (co_await session.poll_send(1) == 0 && session.send_channel().outstanding() != 0) [[unlikely]]
        {
...
NBIO::Task<ReportStatus> ReadReport(NBIO::RDMA_Session &session, std::uint64_t packets, bool wait, Progress &progress)
{
    std::optional<std::span<char>> message;
    if (wait)
    {
        message = co_await session.receive();
    }
    else
    {
        message = co_await session.try_receive();
    }
    if (!message)
    {
        if (wait)
...
// before one arrived.
NBIO::Task<bool> WaitForReport(NBIO::RDMA_Session &session, std::uint64_t packets, Progress &progress)
{
...
NBIO::Task<bool> HarvestReports(NBIO::RDMA_Session &session, std::uint64_t packets, Progress &progress)
{
    while (true)
    {
...
TransferOffer ReceiverOffer(PayloadKind kind, std::uint32_t chunk_size, std::uint64_t offset) noexcept
{
    ...
    return {.kind = kind,
            ...
            .num_chunks = Foundation::Core::RDMA_Stream::kReceiveChunks,
            .offset = offset};
}
...
std::uint32_t NegotiatedWindow(const TransferOffer &offer) noexcept
{
    ...
    constexpr std::uint32_t sender_receives = Foundation::Core::RDMA_Stream::kReceiveChunks;
...
Foundation::NBIO::Task<std::optional<TransferOffer>> AcceptTransfer(NBIO::RDMA_Session &session)
{
    auto opening = co_await session.receive();
    if (!opening)
    {
...
Foundation::NBIO::Task<std::optional<std::uintmax_t>> SendPayload(NBIO::RDMA_Session &session,
                                                                 ...
                                                                 const PacketReader &read, std::size_t packet_size)
{
    ...
    while (sent < packets)
    {
        ...
        if (const auto failure = session.send(*chunk, filled)) [[unlikely]]
        {
            spdlog::warn("transfer: sending byte {} failed: {}", offset, failure.message());
            co_return std::nullopt;
        }
        offset += filled;
        ++sent;
...
Foundation::NBIO::Task<std::optional<TransferResult>> ReceivePayload(NBIO::RDMA_Session &session, PayloadKind kind,
                                                                    ...
                                                                    std::size_t chunk_size)
{
    ...
    auto plan_message = co_await session.receive();
    if (!plan_message)
    {
    ...
    {
        auto packet = co_await session.receive();
        if (!packet) [[unlikely]]
        {
...
Foundation::NBIO::Task<std::optional<std::uintmax_t>> SendFile(NBIO::RDMA_Session &session,
...
Foundation::NBIO::Task<std::optional<std::uintmax_t>> SendFile(NBIO::RDMA_Session &session, const TransferOffer &offer,
                                                               const std::filesystem::path &path,
...
Foundation::NBIO::Task<std::optional<TransferResult>> ReceiveFile(NBIO::RDMA_Session &session,
                                                                  const std::filesystem::path &path,
                                                                  std::size_t chunk_size)
{
...
Foundation::NBIO::Task<std::optional<std::uintmax_t>> SendCommands(NBIO::RDMA_Session &session,
...
Foundation::NBIO::Task<std::optional<TransferResult>> ReceiveCommands(NBIO::RDMA_Session &session,
                                                                      std::uint64_t offset,
