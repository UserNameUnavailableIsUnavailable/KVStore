...
#include <Application/Server/RDMA_Transfer.hpp>
                           ...
                           Outcome &sender, Outcome &receiver)
{
    ...
    Core::RDMA_Acceptor acceptor(std::move(receive_pool), std::move(send_pool));
    acceptor.listen(Core::Address::from_ipv4(address, port));
    ...
    std::thread sending([&] {
        RunEngine(
            [&] {
                NBIO::RDMA_AcceptChannel channel(acceptor, NBIO::Engine::multiplexer(), NBIO::Engine::scheduler());
    ...
    std::thread receiving([&] {
        RunEngine(
            [&] {
                Core::RDMA_Connector connector(Core::BitmapMemory(receiver_chunk, kChunks),
                                               Core::BitmapMemory(receiver_chunk, kChunks));
                NBIO::RDMA_ConnectChannel channel(connector, NBIO::Engine::multiplexer(), NBIO::Engine::scheduler());
                NBIO::run([&]() -> NBIO::Task<void> {
                    auto session = co_await channel.connect(Core::Address::from_ipv4(address, port));
                           ...
                           Outcome &sender, Outcome &receiver)
{
    ...
    std::thread receiving([&] {
        RunEngine(
            [&] {
                Core::RDMA_Connector connector(Core::BitmapMemory(kPacketSize, kChunks),
...
TEST(RDMA_TransferTesting, PacketCountRoundsUp)
{
...
TEST(RDMA_TransferTesting, NothingIsZeroPackets)
{
...
TEST(RDMA_TransferTesting, OpeningCarriesTheKindAndWhatTheReceiverCanTake)
{
...
TEST(RDMA_TransferTesting, TheKindWordIsWhatSaysWhatThePayloadIs)
{
    KV::TransferOffer offer;
...
TEST(RDMA_TransferTesting, TheMessagesAreToldApart)
{
...
TEST(RDMA_TransferTesting, PlanCarriesWhatTheSenderDecided)
{
...
TEST(RDMA_TransferTesting, CreditCarriesTheCountWritten)
{
...
TEST(RDMA_TransferTesting, DoneCarriesTheBytesWritten)
{
...
TEST(RDMA_TransferTesting, TheNumbersAreInNetworkOrder)
{
...
TEST(RDMA_TransferTesting, TheOfferIsWhatTheReceiverCanHold)
{
    const auto offer = ReceiverOffer(KV::PayloadKind::kCommands, kSmallPacketSize);
    EXPECT_EQ(offer.kind, KV::PayloadKind::kCommands);
    EXPECT_EQ(offer.chunk_size, kSmallPacketSize);
    EXPECT_EQ(offer.num_chunks, Core::RDMA_Stream::kReceiveChunks);
    ...
    EXPECT_EQ(NegotiatedWindow(offer), Core::RDMA_Stream::kReceiveChunks);
}

TEST(RDMA_TransferTesting, TheWindowIsTheSmallerOfTheTwoReceiveCounts)
{
    ...
    EXPECT_EQ(NegotiatedWindow(generous), Core::RDMA_Stream::kReceiveChunks);
...
TEST(RDMA_TransferTesting, SendsAFileAcrossTheWire)
{
    SetLogLevelFromEnvironment();

    const std::string address = RdmaAddress();
    if (address.empty())
    {
...
TEST(RDMA_TransferTesting, SendsWithTheSmallerOfTheTwoChunkSizes)
{
...
TEST(RDMA_TransferTesting, SendsCommandsAcrossTheWire)
{
...
TEST(RDMA_TransferTesting, ASnapshotIsRefusedWhereCommandsWereAskedFor)
{
