...
public:
    ...
    explicit LinuxSocket(SocketProtocol protocol)
    {
        ...
        handle_ = Foundation::Socket(AF_INET, type, native_protocol);
        if (handle_ < 0)
        {
    ...
    HandleType get_native_handle() const noexcept
    {
        return handle_;
    }
    ...
    {
        out.Reset();
        out.GetSize() = out.Capacity();
        if (::getsockname(handle_, &out.Storage<::sockaddr>(), &out.GetSize()) < 0)
        {
    ...
    void GetPeerAddress(Address& out) const
    {
        out.Reset();
        out.GetSize() = out.Capacity();
        if (::getpeername(handle_, &out.Storage<::sockaddr>(), &out.GetSize()) < 0)
        {
    ...
    void set_non_blocking(bool enabled = true)
    {
        ...
        if (flags < 0 || ::fcntl(handle_, F_SETFL, flags) < 0)
        {
