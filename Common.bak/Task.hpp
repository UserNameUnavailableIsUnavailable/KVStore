...
public:
    ...
    Socket::HandleType get_handle() const noexcept
    {
        return session_.GetSocket().get_native_handle();
    }
    std::span<char> Get::Foundation::Buffer() const noexcept { return buffer_; }
...
public:
    ...
    std::span<const char> Get::Foundation::Buffer() const noexcept { return buffer_; }
    std::coroutine_handle<> GetContinuation() const noexcept { return continuation_; }
    void Complete(int result) noexcept { result_ = result; }
