...
public:
    ...
    KV::Message wait() override
    {
        while (true)
        {
            ...
            if (!waitReadable(timers_.get_native_handle(), 5s))
            {
                return {.type = KV::MessageType::kTimer,
                    .session_id = KV::Message::kNoSession,
                    .result = 0,
...
TEST(TimerTesting, StartsDisarmed)
{
    ...
    EXPECT_GE(timer.get_native_handle(), 0);
...
TEST(TimerTesting, ArmsOnConstructionAndFires)
{
    ...
    ASSERT_TRUE(waitReadable(timer.get_native_handle(), 2s));
...
TEST(TimerTesting, DoesNotFireBeforeItsDeadline)
{
    ...
    EXPECT_FALSE(waitReadable(timer.get_native_handle(), 20ms));
...
TEST(TimerTesting, AcceptsAnyDurationUnit)
{
    ...
    EXPECT_TRUE(waitReadable(nanoseconds.get_native_handle(), 2s));
    EXPECT_TRUE(waitReadable(microseconds.get_native_handle(), 2s));
    EXPECT_TRUE(waitReadable(milliseconds.get_native_handle(), 2s));
}
...
TEST(TimerTesting, IntervalFiresRepeatedly)
{
    ...
    {
        ASSERT_TRUE(waitReadable(timer.get_native_handle(), 2s)) << "tick " << tick;
        EXPECT_GE(timer.Drain(), 1u);
    }
}
...
TEST(TimerTesting, DisarmStopsAPendingTimer)
{
    ...
    EXPECT_FALSE(timer.IsArmed());
    EXPECT_FALSE(waitReadable(timer.get_native_handle(), 80ms));
}
...
TEST(TimerTesting, MoveTransfersTheDescriptor)
{
    ...
    const KV::Timer::HandleType handle = source.get_native_handle();

    KV::Timer moved(std::move(source));

    EXPECT_EQ(moved.get_native_handle(), handle);
    EXPECT_FALSE(source.IsOpen());
    ASSERT_TRUE(waitReadable(moved.get_native_handle(), 2s));
...
TEST(TimerQueueTesting, StartsEmptyWithOneDescriptor)
{
    ...
    EXPECT_GE(queue.get_native_handle(), 0);
...
TEST(DelayedOperationTesting, OneExpirationCanReleaseAWholeBatch)
{
    ...
    ASSERT_TRUE(waitReadable(queue.GetTimerQueue().get_native_handle(), 2s));
