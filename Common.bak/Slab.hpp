...
public:
    ...
    void ForEach(Visitor&& visit)
    {
        ...
        {
            Slot& slot = slot(static_cast<IndexType>(index));
            if (!slot.occupied)
            {
                continue;
            }
    ...
    void ForEach(Visitor&& visit) const
    {
        for (std::size_t index = 0; index < Capacity(); ++index)
        {
            const Slot& slot = slot(static_cast<IndexType>(index));
            if (!slot.occupied)
            {
                continue;
            }
...
private:
    ...
    Slot& slot(IndexType index) noexcept
    {
        return blocks_[index / SlotsPerBlock]->slots[index % SlotsPerBlock];
    }

    const Slot& slot(IndexType index) const noexcept
    {
        return blocks_[index / SlotsPerBlock]->slots[index % SlotsPerBlock];
    }
    ...
    const Slot* resolve(Handle handle) const noexcept
    {
        ...
        const Slot& slot = slot(handle.index);
