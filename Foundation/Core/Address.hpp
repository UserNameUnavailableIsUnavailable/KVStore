...
// kernel fill it in through `Bind`/`Accept`/`GetPeerAddress`.
class Address
{
  public:
    enum class Family
    ...
    static Address from_ipv4(std::string_view ip, std::uint16_t port);

    static Address from_ipv6(std::string_view ip, std::uint16_t port);
