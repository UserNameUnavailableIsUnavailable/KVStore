...
TcpSocket::TcpSocket(SocketAddress::Family family, Type type)
{
    EnsureWinsock();
    ...
    switch (family)
    {
    case SocketAddress::Family::kIPv4:
        domain = AF_INET;
        break;
    case SocketAddress::Family::kIPv6:
        domain = AF_INET6;
        break;
    default:
    ...
    case Type::kDatagram:
        ...
        break;
    default:
        throw std::system_error(std::make_error_code(std::errc::protocol_not_supported), "TcpSocket: unsupported type");
    }

    // Foundation::TcpSocket is shared in Linux and Windows
...
expected<void, std::error_code> TcpSocket::bind(const SocketAddress &local) noexcept
{
...
expected<void, std::error_code> TcpSocket::listen(int backlog) noexcept
{
...
expected<std::pair<TcpSocket, SocketAddress>, std::error_code> TcpSocket::accept() noexcept
{
    SocketAddress address;
    while (true)
    {
        address.length() = SocketAddress::capacity();
...
expected<std::size_t, std::error_code> TcpSocket::receive(std::span<char> buffer) noexcept
{
    while (true)
    {
        ...
        if (n == 0)
        {
            return 0; // the peer closed the stream
        }
        if (IS_SOCKET_ERROR_AGAIN)
        {
            return unexpected<std::error_code>(std::make_error_code(std::errc::operation_would_block));
...
expected<std::size_t, std::error_code> TcpSocket::send(std::span<const char> buffer) noexcept
{
    while (true)
    {
...
expected<void, std::error_code> TcpSocket::connect(const SocketAddress &peer) noexcept
{
...
expected<void, std::error_code> TcpSocket::non_blocking(bool toggle) noexcept
{
#if defined(__linux__)
    ...
    {
        return unexpected<std::error_code>(get_last_error());
    }
#endif
    return {};
}
...
expected<void, std::error_code> TcpSocket::reuse_address(bool toggle) noexcept
{
    const int value = toggle ? 1 : 0;
    if (auto error = set_native_option(SOL_SOCKET, SO_REUSEADDR, value))
    {
        return unexpected<std::error_code>(std::move(error));
    }
    return {};
}

expected<void, std::error_code> TcpSocket::reuse_port(bool toggle) noexcept
{
#if defined(SO_REUSEPORT)
    const int value = toggle ? 1 : 0;
    if (auto error = set_native_option(SOL_SOCKET, SO_REUSEPORT, value))
    {
        return unexpected<std::error_code>(std::move(error));
    }
#endif
...
expected<void, std::error_code> TcpSocket::keep_alive(bool enable) noexcept
{
    const int value = enable ? 1 : 0;
    if (auto error = set_native_option(SOL_SOCKET, SO_KEEPALIVE, value))
    {
        return unexpected<std::error_code>(std::move(error));
    }
    return {};
...
expected<SocketAddress, std::error_code> TcpSocket::local_address() const noexcept
{
    SocketAddress address;
    address.length() = SocketAddress::capacity();
    if (::getsockname(handle_, address.storage<sockaddr>(), &address.length()) < 0) {
        return unexpected<std::error_code>(TcpSocket::last_error());
    }
    return address;
}

expected<SocketAddress, std::error_code> TcpSocket::peer_address() const noexcept
{
    SocketAddress address;
    address.length() = SocketAddress::capacity();
    if (::getpeername(handle_, address.storage<sockaddr>(), &address.length()) < 0)
    {
        return unexpected(TcpSocket::last_error());
    }
    return address;
}
