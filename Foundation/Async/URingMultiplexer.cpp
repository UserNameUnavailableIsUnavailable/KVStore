...
bool URingMultiplexer::prepare(Channel *channel)
{
...
void URingMultiplexer::complete(Channel *channel, int result)
{
    switch (channel->type())
    {
    case ChannelType::kReceive: {
...
void URingMultiplexer::handle_completions()
{
    ...
    {
        auto *channel = static_cast<Channel *>(::io_uring_cqe_get_data(cqe));
        if (channel != nullptr)
        {
...
void URingMultiplexer::update_channel(Channel *channel)
{
